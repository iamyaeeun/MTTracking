...
// Since host machine has enough GPU performance, it doesn't need to be shriked
#if defined(PERF_INFERENCE_PRECISION_FP16) && !defined(_WIN32)
	m_localConfig.inference_precision = tensor_rt::FP16;
#else
...
void YoloTensorRTDetector::Detect(const cv::UMat& colorFrame)
{
    ...
    if (m_maxCropRatio <= 0)
    {
		...
		t4 = cv::getTickCount();
		std::cout << "mat: " << (1000 * (t2 - t1) / freq) << ", det: " << (1000 * (t3 - t2) / freq) << ", rgn: " << (1000 * (t4 - t3) / freq) << std::endl;
	}
    else
    {
...
///
/// \brief YoloTensorRTDetector::DetectMat
/// \param gray
///
void YoloTensorRTDetector::DetectMat(cv::Mat frame)
{
	//double freq = cv::getTickFrequency();
	//int64 t1, t2, t3, t4;
	//t1 = cv::getTickCount();
