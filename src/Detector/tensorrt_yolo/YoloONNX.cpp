                                ...
                                YoloONNXUniquePtr<nvonnxparser::IParser>& parser)
{
    ...
    int verbosity = (int)nvinfer1::ILogger::Severity::kVERBOSE; // kERROR;
    ...
    // _lkh you may see the below message when use yolov7 onnx model
    // "[TRT] Some tactics do not have sufficient workspace memory to run. 
    // Increasing workspace size will enable more tactics, please check verbose output for requested sizes."
    // Increase size of the memory by 512MB
    ...
    config->setMemoryPoolLimit(MemoryPoolType::kWORKSPACE, 4608_MiB);
#else
    //config->setMaxWorkspaceSize(4096_MiB);
    config->setMaxWorkspaceSize(4608_MiB);
#endif
