...
void Yolo::doInference(const unsigned char* input, const uint32_t batchSize)
{
	...
	//int64 t1, t2, t3, t4, t5;

	//Timer timer;
    assert(batchSize <= m_BatchSize && "Image batch size exceeds TRT engines batch size");

// _lkh improvement: cudaStreamSynchronize consumes huge amount of time for synchronization
// if we move this sychronization before starting stream, can reduce certain amount of time
// Finally, can achieve 30 fps with 720p from this change
#ifndef PERF_CUDA_POST_SYNC
	//t4 = cv::getTickCount();
...
#ifdef PERF_CUDA_POST_SYNC
	//t4 = cv::getTickCount();
	cudaStreamSynchronize(m_CudaStream);
	//t5 = cv::getTickCount();
#endif
	//std::cout << "inference h2d: " << (1000*(t2 - t1) / freq) << ", execute: " << (1000 * (t3 - t2) / freq) << ", d2h: " << (1000 * (t4 - t3) / freq) << ", sync: " << (1000 * (t5 - t4) / freq) << std::endl;
