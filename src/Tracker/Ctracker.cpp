...
void CTracker::Update(const regions_t& regions, cv::UMat currFrame, float fps)
{
    ...
    // _lkh improvement: If Kalman filter is not using for object size detection,
    // don't need to back up the current image
    if (m_settings.m_filterGoal == tracking::FilterRect)
    {
...
#pragma omp parallel for
        ...
        {
            cv::rectangle(dbgAssignment, reg.m_brect, cv::Scalar(0, 255, 255), 1, cv::LINE_4);
        }
    }
#endif
    ...
    if (!m_tracks.empty())
    {
        ...
        for (size_t i = 0; i < assignment.size(); i++)
        {
            ...
            if (assignment[i] != -1)
            {
                ss << m_tracks[i]->GetID().ID2Str() <<": " << std::fixed << std::setprecision(2) << costMatrix[i + assignment[i] * N];

                // In this case there is no detected object to this track
            ...
            {
                ss << m_tracks[i]->GetID().ID2Str() << ": ";

                if (m_tracks[i]->GetID() == 8)
                {
                    cv::rectangle(dbgAssignment, m_tracks[i]->LastRegion().m_brect, cv::Scalar(255, 0, 255), 1);
                }

                // If track have no assigned detect, then increase skipped frames counter
                cv::rectangle(dbgAssignment, m_tracks[i]->LastRegion().m_brect, cv::Scalar(255, 0, 255), 1);
                for (size_t ri = 0; ri < regions.size(); ++ri)
                {
        ...
        for (size_t i = 0; i < m_tracks.size();)
        {
            if (m_tracks[i]->SkippedFrames() > m_settings.m_maximumAllowedSkippedFrames ||
                    ...
                    //m_tracks[i]->IsStaticTimeout(cvRound(fps * (m_settings.m_maxStaticTime - m_settings.m_minStaticTime))))   // _lkh test
                    m_tracks[i]->IsStaticTimeout(20))
            {
...
#pragma omp parallel for
    ...
    num << "detect/track/assign: " << regions.size() << "/" << m_tracks.size() << "/" << assignment.size();
    cv::putText(dbgAssignment, num.str(), cv::Point(10, 20), cv::FONT_HERSHEY_DUPLEX, 0.7, cv::Scalar(10, 10, 10), 1, 8);
                                   ...
                                   track_t& maxCost)
{
    ...
    for (size_t i = 0; i < N; ++i)
    {
        ...
        for (size_t j = 0; j < regions.size(); ++j)
        {
            ...
            if constexpr (DIST_LOGS)      // _lkh debug
