...
public:
    ...
    void Draw(cv::Mat frame) const
    {
        ...
        cv::line(frame, Ptf2i(m_pt1), Ptf2i(m_pt2), lineColor, 1, cv::LINE_8, 0);

        std::string label = "L-" + std::to_string(m_uid) + ": " + std::to_string(m_intersect1) + "/" + std::to_string(m_intersect2);
		int baseLine = 0;
        double fontScale = 0.5;
		int thickness = 1;
        cv::Size labelSize = cv::getTextSize(label, cv::FONT_HERSHEY_SIMPLEX, fontScale, 1, &baseLine);
        //cv::Point pt(Ptf2i(0.5f * (m_pt1 + m_pt2)));
        cv::Point pt(Ptf2i(m_pt1));
        pt.y += labelSize.height;
		//pt.x += labelSize.width;
        cv::putText(frame, label, pt, cv::FONT_HERSHEY_TRIPLEX, fontScale, cv::Scalar(255, 255, 255), thickness);
    }
